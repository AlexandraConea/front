...
export class PokerService {
  ...
  raise(py: Player, bet: number): Player {
    py = this.bet(py, bet);
    this.variables.currentBet = bet;
  ...
  makeMoveForRobot(move: string, robot: Player): Player {
    if (move == "raise") {
      let money = Math.floor(Math.random() * 50 + this.variables.currentBet + 1);
      robot = this.raise(robot, money);
    }
    if (move == "check") {
