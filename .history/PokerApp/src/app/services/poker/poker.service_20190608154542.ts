...
export class PokerService {
  ...
  async blindRound(){
    ...
    for(let i=0;i<7;i++){
      ...
      this.robotService.blindMove(i.toString(),{cards: this.variables.robots[i].hand}).subscribe(data => { move = data; });
      while (move == null) {
        await delay(1000);
      }
  ...
  async flopRound() {
    ...
    for (let robot of this.variables.robots) {
      move = null;
      await delay(1500);
      this.robotService.flopMove((robot.id - 1).toString(),{cards:this.variables.flop}).subscribe(data => { move = data; });
      while (move == null) {
        await delay(1000);
      }
  ...
  async turnRound() {
    ...
    for (let robot of this.variables.robots) {
      move = null;
      await delay(1500);
      this.robotService.turnMove((robot.id - 1).toString(),{cards:this.variables.flop}).subscribe(data => { move = data; });
      while (move == null) {
        await delay(1000);
      }
      robot = this.makeMoveForRobot(move, robot);
  ...
  async riverRound() {
    ...
    for (let robot of this.variables.robots) {
      move = null;
      await delay(1500);
      this.robotService.riverMove((robot.id - 1).toString(),{cards:this.variables.flop}).subscribe(data => { move = data; });
      while (move == null) {
        await delay(1000);
      }
      robot = this.makeMoveForRobot(move, robot);
