...
export class PokerService {
  ...
  async showdown() {
    ...
    console.log("heere")
    this.robotService.evaluate(json).subscribe(data => { winner = data['body']; console.log(winner) });
    while (winner == null)
      await delay(1000);
    if (winner['player'] == 'You') {
      this.variables.human.amount += this.variables.pot;
    }
    this.variables.history.push(winner['player'] + " won.")
    this.variables.flop = winner['cards'];
  }
