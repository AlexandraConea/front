...
export class PokerService {
  ...
  fold(py: Player): Player {
    ...
    this.variables.playerNumberRound-=1;
    if (py.isRobot)
      return py;
    else {
  ...
  filterFolded(): void {
    ...
    this.variables.playerNumberRound = this.variables.playerNumber;
  }
  ...
  async flopRound() {
    ...
    for (let robot of this.variables.robots) {
      move = null;
      await delay(1000);
      this.robotService.flopMove(robot.id.toString(), { cards: this.variables.flop }).subscribe(data => { move = data['body']; });
      while (move == null) {
        await delay(1000);
      }
      let index = this.variables.robots.indexOf(robot);
      this.variables.robots[index] = this.makeMoveForRobot(move, robot);
      await delay(1000);
    }
  }
  ...
  async turnRound() {
    ...
    for (let robot of this.variables.robots) {
      if (this.variables.playerNumberRound == 1) {
        this.showdown();
        return;
      }
      move = null;
      await delay(1500);
      this.robotService.turnMove(robot.id.toString(), { cards: this.variables.flop }).subscribe(data => { move = data['body']; });
      while (move == null) {
        await delay(1000);
      }
      let index = this.variables.robots.indexOf(robot);
  ...
  async riverRound() {
    ...
    for (let robot of this.variables.robots) {
      ...
      move = null;
      await delay(1500);
      this.robotService.riverMove(robot.id.toString(), { cards: this.variables.flop }).subscribe(data => { move = data['body']; });
      while (move == null) {
        await delay(1000);
      }
      let index = this.variables.robots.indexOf(robot);
  ...
  async flopRound2() {
    ...
    for (let robot of this.variables.robots) {
      move = null;
      await delay(1000);
      this.robotService.flopMove2(robot.id.toString(), { cards: this.variables.flop }).subscribe(data => { move = data['body']; });
      while (move == null) {
        await delay(1000);
      }
      let index = this.variables.robots.indexOf(robot);
  ...
  async turnRound2() {
    ...
    for (let robot of this.variables.robots) {
      ...
      move = null;
      await delay(1500);
      this.robotService.turnMove2(robot.id.toString(), { cards: this.variables.flop }).subscribe(data => { move = data['body']; });
      while (move == null) {
        await delay(1000);
      }
      let index = this.variables.robots.indexOf(robot);
  ...
  async riverRound2() {
    ...
    for (let robot of this.variables.robots) {
      ...
      move = null;
      await delay(1500);
      this.robotService.riverMove2(robot.id.toString(), { cards: this.variables.flop }).subscribe(data => { move = data['body']; });
      while (move == null) {
        await delay(1000);
      }
      let index = this.variables.robots.indexOf(robot);
  ...
  processHand(handString: string): Card[]{
    ...
    for (let card of cards){
      ...
      let color:string;
      if(cardData[1]=='diamonds' || cardData[1]=="hearts")
        color = "#DE3A39";
      else
        color = 'black';
      let number;
      if(cardData[0]=='T')
        ...
        number=cardData[0];
      let c = new Card(cardData[1],number,color);
      c.faceUp = true;
      winningHand.push(c);
    }
    return winningHand;
  ...
  async showdown() {
    ...
    console.log(player);
    if (player == 'You') {
      this.variables.human.amount += this.variables.pot;
    }
    this.variables.history.push(player + " won.")
    let winningHand = this.processHand(winner.cards);
    winningHand.sort((a, b) => (a.number > b.number) ? 1 : -1)
    this.variables.showdown = true;
    this.variables.flop = winningHand;
