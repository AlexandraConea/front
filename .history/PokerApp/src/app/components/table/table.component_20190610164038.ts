...
export class TableComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    interval(500).subscribe(() => {
      ...
      this.updateScroll();
      this.showdown = this.variables.showdown;
    })
  ...
  public leave(): void {
    ...
    this.userService.update(this.storage.getLoggedUser().userId, this.storage.getLoggedUser().moneyAmount).subscribe(()=>{});
    this.video.pause();
    this.video.srcObject.getTracks()[0].stop();
    this.gameService.deleteGame(this.id).subscribe(() => { });
    this.router.navigate(['/home'],
      { queryParams: { rp: new Date().getTime() } });
