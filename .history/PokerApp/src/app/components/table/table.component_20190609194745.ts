...
export class TableComponent implements OnInit {
  ...
  constructor(private pokerService: PokerService, private router: Router, private storage: AuthDataStorage, 
    private route: ActivatedRoute, private gameService: GamesService,private userService: LoginFormService) {
  ...
  ngOnInit() {
    ...
    interval(500).subscribe(() => {
        ...
        this.updateScroll(scrolled);
        var scrolled: boolean;
        $("scroll-me").on('scroll', function(){
          scrolled=true;
      });
    })
  ...
  public leave(): void{
    ...
    this.userService.update(this.storage.getLoggedUser().userId, this.storage.getLoggedUser().moneyAmount);
    this.video.pause();
